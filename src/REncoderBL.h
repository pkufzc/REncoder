...
unordered_map<uint64_t,uint64_t> black_list;
...
public:
    ...
    YRENCODER(uint64_t _hash_num, uint64_t _memory, uint64_t _L, uint64_t _stored_level, RangeBloomfilter *_rbf)
    {
        hash_num = _hash_num;
        memory = _memory;
        L = _L;
    ...
    static pair<YRENCODER *, size_t> deserialize(uint8_t *ser)
    {
        uint8_t *pos = ser;
        ...
        return {new YRENCODER(hash_num, memory, L, stored_level, tmp.first), len};
    }
    ...
    uint64_t black_list_key, black_list_value;
    ...
    bool RangeQuery(uint64_t low, uint64_t high, uint64_t p = 0, uint64_t l = 1)
    {
        ...
        black_list_key = black_list_value = 0;
        first = 0;
        // Query [0,2^64-1].
        if (leftBorder && rightBorder)
        {
            if (verify(0, l + 1))
            {
               // if (!check_black_list_value(0,l))
                   return true;
            }
            return verify(1ll << (L - l), l + 1);// && (!check_black_list_value(1ll<<(L - l), l));
        }
    ...
    bool verify(uint64_t p, uint64_t l)
    {
        ...
        while (!prefixes.empty())
        {
            ...
            if (l > L)
            {
                if(check_black_list_value(p,l-1))
                    continue;
                // Reach a leaf node.
                return true;
            }
...
#endif
    bool check_black_list_value(uint64_t p, uint64_t l) {
        
        if ((p >> 6) != black_list_key || first == 0) {
            black_list_key = p >> 6;
            auto it = black_list.find(black_list_key);
            if (it != black_list.end()) {
                black_list_value = it->second; 
                first = 1;
            }
            else {
                first = 2;
        ...
        uint64_t test_value; 
        test_value =(0x1llu << (p & 0b111111));
            //printf("%lu %lu %lu %u\n",p>>6,value,test_value,test_value & value == test_value);
        return (test_value & black_list_value) == test_value;
        /*
        switch (l - 58)
        {
        
        case 0:
            test_value = (0xFFFFffffFFFFffffllu);
            return (test_value & black_list_value) == test_value;
        case 1:
            test_value = (0xFFFFffffllu << (p & 0b100000));
            return (test_value & black_list_value) == test_value;
        case 2: 
            test_value =(0xFFFFllu << (p & 0b110000));
            return (test_value & black_list_value) == test_value;
        case 3:
            test_value =(0xFFllu << (p & 0b111000));
            return (test_value & black_list_value) == test_value;
        case 4:
            test_value =(0xFllu << (p & 0b111100));
            return (test_value & black_list_value) == test_value;
        case 5:
            test_value =(0x3llu << (p & 0b111110));
            return (test_value & black_list_value) == test_value;
        case 6:
            test_value =(0x1llu << (p & 0b111111));
            //printf("%lu %lu %lu %u\n",p>>6,value,test_value,test_value & value == test_value);
    ...
    void get_black_list_value(uint64_t p, uint64_t l) { 
        
        if ((p >> 6) != black_list_key) {
            black_list[black_list_key] |= black_list_value;
            black_list_value = 0;
            black_list_key = p >> 6;
        }
        switch (l - 58)
        {
        case 2:
            black_list_value |= (0xFFFFllu << (p & 0b110000));  
            return;
            
        case 0: 
            black_list_value |= (0xFFFFffffFFFFffffllu);  
            return;
        case 1:
            black_list_value |=  (0xFFFFffffllu << (p & 0b100000));  
            return;
        case 3:
            black_list_value |= (0xFFllu << (p & 0b111000));  
            return;
        case 4:
            black_list_value |= (0xFllu << (p & 0b111100));  
            return;
        case 5:
            black_list_value |= (0x3llu << (p & 0b111110));  
            return; 
        default:
            black_list_value |= (0x1llu << (p & 0b111111));  
            return; 
        }

    }
    void add_black_list_node(uint64_t low, uint64_t high, uint64_t p = 0, uint64_t l = 1)
    {
        // Point query.
        if (low == high)
        { 
        ...
        black_list_value = black_list_key = 0;
        // Query [0,2^64-1].
        if (leftBorder && rightBorder)
        {
    ...
    uint64_t query_black_list_size() {
        uint64_t res = black_list.size();
        return  res;
    }
